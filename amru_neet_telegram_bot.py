...
def ensure_file_exists(path, default_content):
    """Create the file with default content if it doesn't exist yet,
    so the save-progress step always has something valid to commit."""
...
def main():
    ...
    if IS_MANUAL_RUN:
        ...
        send_telegram_message(
            f"🟢 Still running fine — {now_str}\nTracking {len(new_seen)} notices. No new counselling notice yet."
        )
        mark_heartbeat_sent()
